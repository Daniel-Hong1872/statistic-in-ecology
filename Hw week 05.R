...
for(i in 1:n_p){
  ...
  com_shuffle <- combine[order(runif(length(combine)))]
  Onve_shuffle <- com_shuffle[1:length(Onve_den)]
  Capa_shuffle <- com_shuffle[(length(Onve_den) + 1):length(com_shuffle)]
  
  diff_null[i] <- mean(Onve_shuffle) - mean(Capa_shuffle)
}
